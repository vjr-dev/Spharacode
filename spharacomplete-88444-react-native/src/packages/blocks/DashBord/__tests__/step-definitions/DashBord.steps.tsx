...
import { Platform, reactNative, Alert, AppState, Linking, View } from "react-native";
...
defineFeature(feature, (test) => {
  ...
  test("User navigates to DashBord", ({ given, when, then }) => {
    ...
    then('renders correctly when ModalShow is true', () => {
     ...
     // console.log('PRIYTAMADD====>',wrapper.state().imageWidth)
    ...
    then('should handle volumeEventFunction correctly', () => {
      const wrapper = shallow(<Dashbord {...screenProps}/>);
    ...
    then("I can press emergency screen with out errror", () => {
     ...
     // emergencyScreenComponent.props().onPress();
    });
    ...
    then("I can mock VolumeEvents with out error", () => {
      ...
      instance.setState({ volumeUpTriggerCount: 2 });
      instance.VolumeEvents();

      instance.setState({volumeDownTriggerCount: 1})
    });

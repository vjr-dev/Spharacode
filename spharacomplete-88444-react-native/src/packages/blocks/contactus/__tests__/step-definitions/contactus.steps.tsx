...
defineFeature(feature, test => {
  ...
  test("User navigates to addContactus", ({ given, when, then }) => {
    ...
    when("I navigate to the addContactus", () => {
      ...
      let buttonComponent = AddContactUsWrapper.findWhere((node) => node.prop('testID') === 'btnSubmit');
      buttonComponent.simulate('press');
     
      let textInputComponent = AddContactUsWrapper.findWhere((node) => node.prop('testID') === 'txtName');
      textInputComponent.simulate('changeText', 'FIRST');

      textInputComponent = AddContactUsWrapper.findWhere((node) => node.prop('testID') === 'txtEmail');
      textInputComponent.simulate('changeText', 'a@b.com');

      textInputComponent = AddContactUsWrapper.findWhere((node) => node.prop('testID') === 'txtPhoneNumber');
      textInputComponent.simulate('changeText', '13105551212');

      textInputComponent = AddContactUsWrapper.findWhere((node) => node.prop('testID') === 'txtComments');
    ...
    then("addContactus will load with out errors", () => {
      ...
      expect(AddContactUsWrapper).toMatchSnapshot();
    });
